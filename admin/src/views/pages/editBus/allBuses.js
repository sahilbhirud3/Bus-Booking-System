...
const ShowBuses = () => {
  ...
  const handleDownload = () => {
    ...
    pdf.setFontSize(8);
    pdf.text(`Site Name: ${siteName}  |  Download Date: ${downloadDate}`, 110, 10);
    ...
    const startY = 50;
      ...
      pdf.text(`Start Time: ${new Date(passengerList.startTime).toLocaleString()}`, startX+5, startY + 40);
      pdf.text(`End Time: ${new Date(passengerList.endTime).toLocaleString()}`, startX+5, startY + 50);
      pdf.text(`Total Seats: ${passengerList.totalSeats}`, startX+5, startY + 60);
    ...
    for (let seatNo = 1; seatNo <= passengerList.totalSeats; seatNo++) {
      const passenger = passengerList.seatList.find(seat => seat.seatNo === seatNo);
      if (passenger) {
        tableData.push([
          passenger.seatNo,
  ...
  for (let i = 1; i <= totalPages; i++) {
      pdf.setPage(i);
      pdf.setFontSize(10);
      pdf.text(`Page ${i} of ${totalPages}`, pdf.internal.pageSize.width / 2, pdf.internal.pageSize.height - 10, 'center');
      pdf.text('This is a system-generated PDF. For support contact : +91 8600386608', pdf.internal.pageSize.width / 2, pdf.internal.pageSize.height - 5, 'center');
  }
    // Save the PDF
